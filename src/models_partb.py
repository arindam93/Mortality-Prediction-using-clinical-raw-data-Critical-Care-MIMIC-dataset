...
def logistic_regression_pred(X_train, Y_train):
    ...
    log_pred = LogisticRegression(random_state=545510477)
    log_pred.fit(X_train, Y_train)
    return log_pred.predict(X_train)
...
def svm_pred(X_train, Y_train):
    ...
    svm_pred = LinearSVC(random_state=545510477)
    svm_pred.fit(X_train, Y_train)
    return svm_pred.predict(X_train)
...
def decisionTree_pred(X_train, Y_train):
	#train a logistic regression classifier using X_train and Y_train. Use this to predict labels of X_train
	#use max_depth as 5
    dec_tree_pred = DecisionTreeClassifier(max_depth=5, random_state=545510477)
    dec_tree_pred.fit(X_train, Y_train)   
    return dec_tree_pred.predict(X_train)
